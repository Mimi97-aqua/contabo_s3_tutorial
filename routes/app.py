...
def upload_file():
    ...
    if not file_type:
        return jsonify(
            {"status": "error", "message": "The uploaded file must be a media file."}
        ), 400

    if not size:
        return jsonify({"status": "error", "message": "File is more than 5MB"}), 400

    file_url = (
        f"{os.environ.get('ENDPOINT_URL')}/{os.environ.get('BUCKET_NAME')}/{filename}"
    )
    upload_url = generate_presigned_url(
        method="put_object", key=filename, content_type=content_type
    )

    if not upload_url:
        return jsonify(
            {"satus": "fail", "message": "Upload URL failed to generate"}
        ), 400
    ...
    return jsonify(
        {
            ...
            "message": {
                "file_url": generate_presigned_url(
                    method="get_object", content_type=content_type, key=filename
                )
            },
        }
    ), 200
...
def delete_file():
    ...
    if response.status_code != 204:
        return jsonify(
            {"status": "error", "message": f"Failed to delete file: {response.reason}"}
        ), 400

    return jsonify({"status": "success", "message": "File successfully deleted"}), 200
